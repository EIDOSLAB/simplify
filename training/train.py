...
def main(config):
    ...
    train_loader, test_loader = get_data_loaders(os.path.join(
        config.root, "ImageNet"), 256, 256, 0, True, 8, False)
    ...
    for i in tqdm(range(train_iteration)):
        ...
        if (i + 1) % prune_iteration == 0:
            ...
            for name, module in model.named_modules():
                if isinstance(module, nn.Conv2d):
                    prune.ln_structured(
                        module, 'weight', amount=0.05, n=2, dim=0)
            ...
            print(
                f"The current model has {(remaining_neurons / total_neurons) * 100} % of the original neurons")
            ...
            scheduler = CosineAnnealingLR(
                optimizer, train_iteration, 1e-3, last_epoch=-1)
            for _ in range(i):
                scheduler.step()
